...
library: the current 141-molecule candidate portfolio, after pre-registered
PAINS/BRENK/Lipinski filtering and duplicate removal, contains only 9 unique
drug-like synthetically-credible molecules - a finding the field needs to see,
and the starting point for generative/med-chem fill-in. (2) A demonstration
...
### Preliminary data (all verified in this repository; pipeline runs end-to-end)
...
- Candidate-panel ascorbate linkage: 8/99 prospective candidates preserve the
  ascorbate core; 27/99 carry the open enediol motif (tentative).
...
- SAR: 56 MMP transforms; the 6-bromo-6-deoxy edit (free ascorbate 1797 uM ->
  ...
  TAF-1/2/4/6 LIMITED.
- Library projection: 26 Bemis-Murcko-distinct chemotype families; 24 novel vs
  training; after PAINS/BRENK/Lipinski + dedup only 9/99 unique molecules
...
### Aim 1 - Chemotype library curation
*Design.* Take the 141-compound portfolio (groups A-F: 1/6/2/29/12/91). Apply,
...
stereo probe set (22 probes from 7 actives).
*Fill-in.* Because only 9 drug-like molecules pass, Aim 1 includes two honest
fill-in strategies: (a) expert medicinal-chemistry expansion of the clean
families by analog enumeration around confirmed actives; (b) REINVENT4 Mol2Mol
generation (stereo-aware prior) - currently NO molecules exist because the
binary is absent; any generated molecules must pass the same filters and are
reported with provenance, not fabricated.
...
### Aim 2 - Two-armed closed-loop experiment
*Design.* Per round: ~15 drug-like compounds (or the full eligible set if
smaller). Arms: model-ranked (top by an openly documented rank score =
...
negative-control balance; no experiment proceeds before approval. Pre-registered
...
### Aim 3 - External validation and SAR packaging
*Design.* (1) External validation: assemble published modulator data for
...
provenance for each row. (3) All claims traceable: each SAR claim links to raw
...
- Year 1: complete library curation (Aim 1), ascorbate sub-series SAR
  enrichment (phase 3b), generate fill-in chemotypes, AF3 binding-model
  validation (co-PI), run round 1 (design, gate, TEVC), ingest + refit v1.
- Year 2: rounds 2-3, pooled refit v2-v3, EF evaluation, go/no-go, external
  data assembly (Aim 3 start), AF3 structure-guided SAR integration (TAF-6).
- Year 3: external validation, AI-ready enriched-SAR export (including ascorbate
  sub-series cards), deposition (resource sharing plan), final report;
